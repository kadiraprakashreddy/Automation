...
export class EntryContextService {
  ...
  private detect(): EntryContext {
    const href =
      (this.document.defaultView as Window | null)?.location?.href ?? '';

    if (href.includes('/gosps/to/termination-rules')) {
      return 'termination-rules';
    }

    // In local development (localhost) default to termination-rules so
    // the create/schedules/details routes are accessible without a gateway.
    const hostname =
      (this.document.defaultView as Window | null)?.location?.hostname ?? '';
